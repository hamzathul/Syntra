...
import { BadRequestError, ForbiddenError, getAuthenticatedUser, type LoggerPort } from "backend-p";
...
export function createCompanyContextMiddleware(
  ...
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    if (typeof companyId !== "string" || companyId.trim() === "") {
      logger?.warn(
        { userId: getAuthenticatedUser(res.locals).id, method: req.method, path: req.originalUrl },
